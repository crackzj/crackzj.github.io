...
function generateTemplate(data = []) {
	...
	try {
		...
		const data = new Uint8Array(Buffer.from(`export default ` + JSON.stringify(menus)))
		writeFile(`${process.cwd()}/.vitepress/theme/sidebar.ts`, data, { signal }).then(() => {
			controller.abort()
		})
	} catch (err) {
		console.log(err);
